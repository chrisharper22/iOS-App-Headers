//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import "UIGestureRecognizerDelegate-Protocol.h"
#import "UIViewControllerAnimatedTransitioning-Protocol.h"
#import "UIViewControllerInteractiveTransitioning-Protocol.h"
#import "UIViewControllerTransitioningDelegate-Protocol.h"

@class IGDirectionalPanGestureRecognizer, IGFNFVideoPlayer, IGShoppingPDPLightboxViewController, NSString, UIView;
@protocol IGShoppingLightboxAnimationControllerDelegate, UIViewControllerContextTransitioning;

@interface IGShoppingPDPLightboxAnimationController : NSObject <UIGestureRecognizerDelegate, UIViewControllerAnimatedTransitioning, UIViewControllerInteractiveTransitioning, UIViewControllerTransitioningDelegate>
{
    _Bool _isPresenting;
    _Bool _isInteracting;
    UIView *_transitioningMediaSnapshot;
    IGShoppingPDPLightboxViewController *_lightboxViewController;
    id <UIViewControllerContextTransitioning> _activeTransitionContext;
    IGDirectionalPanGestureRecognizer *_dismissingPanGestureRecognizer;
    IGFNFVideoPlayer *_videoPlayer;
    UIView *_productSticker;
    UIView *_presentingMediaView;
    UIView *_presentedMediaView;
    id <IGShoppingLightboxAnimationControllerDelegate> _delegate;
}

- (void).cxx_destruct;
@property(nonatomic) __weak id <IGShoppingLightboxAnimationControllerDelegate> delegate; // @synthesize delegate=_delegate;
@property(retain, nonatomic) UIView *presentedMediaView; // @synthesize presentedMediaView=_presentedMediaView;
@property(retain, nonatomic) UIView *presentingMediaView; // @synthesize presentingMediaView=_presentingMediaView;
- (void)_finishAnimationWithIsPresenting:(_Bool)arg1 isCancelling:(_Bool)arg2 velocity:(struct CGPoint)arg3;
- (void)_prepareForAnimationWithTransitionContext:(id)arg1 isPresenting:(_Bool)arg2;
- (void)_didPan:(id)arg1;
- (void)_finishAnimationWithTransitionContext:(id)arg1;
- (void)animateTransition:(id)arg1;
- (double)transitionDuration:(id)arg1;
- (void)startInteractiveTransition:(id)arg1;
- (id)interactionControllerForDismissal:(id)arg1;
- (id)animationControllerForDismissedController:(id)arg1;
- (id)animationControllerForPresentedController:(id)arg1 presentingController:(id)arg2 sourceController:(id)arg3;
- (void)updateViewWithFocusMode:(_Bool)arg1 animated:(_Bool)arg2;
- (void)updateViewWithZoomStatus:(_Bool)arg1;
- (id)initWithLightboxViewController:(id)arg1 videoPlayer:(id)arg2 productSticker:(id)arg3;

// Remaining properties
@property(readonly, nonatomic) long long completionCurve;
@property(readonly, nonatomic) double completionSpeed;
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;
@property(readonly, nonatomic) _Bool wantsInteractiveStart;

@end

