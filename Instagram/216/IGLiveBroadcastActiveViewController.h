//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <UIKit/UIViewController.h>

#import "IGKeyboardObserverFrameChangeDelegate-Protocol.h"
#import "IGLiveAddonConforming-Protocol.h"
#import "IGLiveBroadcastActionSheetControllerDelegate-Protocol.h"
#import "IGLiveBroadcastControllerDelegate-Protocol.h"
#import "IGLiveBroadcastControllerRecordingDelegate-Protocol.h"
#import "IGLiveBroadcastLiveRoomManagerDelegate-Protocol.h"
#import "IGLiveBroadcastResourceViewControllerDelegate-Protocol.h"
#import "IGLiveBroadcastTutorialViewControllerDelegate-Protocol.h"
#import "IGLiveBroadcastViewDelegate-Protocol.h"
#import "IGLiveBroadcastViewerInviteManagerDelegate-Protocol.h"
#import "IGLiveFeedbackControllerDelegate-Protocol.h"
#import "IGLiveInstantDemonetizationWarningViewControllerDelegate-Protocol.h"
#import "IGLiveInviteModeratorViewControllerDelegate-Protocol.h"
#import "IGLiveModeratorRTServiceDelegate-Protocol.h"
#import "IGLiveRightsManagementWarningViewControllerDelegate-Protocol.h"
#import "IGLiveRoomInviteSheetControllerDelegate-Protocol.h"
#import "IGLiveRoomPrivacyConfirmationViewControllerDelegate-Protocol.h"
#import "IGLiveShoppingPinnedProductServiceDelegate-Protocol.h"
#import "IGStoryViewerSectionControllerViewerDelegate-Protocol.h"

@class IGBloksAsyncActionHandler, IGCameraViewController, IGKeyboardObserver, IGLiveAddonCoordinator, IGLiveBroadcastActionSheetController, IGLiveBroadcastCaptureManager, IGLiveBroadcastController, IGLiveBroadcastEventController, IGLiveBroadcastLiveRoomManager, IGLiveBroadcastLogger, IGLiveBroadcastModeratorManager, IGLiveBroadcastResourceViewController, IGLiveBroadcastSetupShoppingConfig, IGLiveBroadcastTutorialViewController, IGLiveBroadcastView, IGLiveBroadcastViewerInviteManager, IGLiveCommentModel, IGLiveCommentViewUpdater, IGLiveDarkOverlayView, IGLiveDebugInfoVideoProcessor, IGLiveDonationWaveManager, IGLiveDonationsDataSource, IGLiveFeedbackController, IGLiveInstantDemonetizationWarningViewController, IGLiveInviteModeratorViewController, IGLiveModeratorRTService, IGLiveRightsManagerService, IGLiveRoomHostBroadcastViewController, IGLiveRoomInviteSheetController, IGLiveShoppingPinnedProductService, IGLiveUserPaySupporterDataSource, IGLiveUserPaySupporterListViewController, IGLiveViewStatusTracker, IGLiveViewerListViewController, IGLiveWaveService, IGSessionTracker, IGStoryConfettiAnimationController, IGUser, IGUserSession, IGViewController, NSArray, NSString, UIView;
@protocol FBCancelable, IGAlertHUDHandling, IGLiveAddonCoordinating, IGLiveBroadcastActiveViewControllerDelegate;

@interface IGLiveBroadcastActiveViewController : UIViewController <IGKeyboardObserverFrameChangeDelegate, IGLiveBroadcastActionSheetControllerDelegate, IGLiveBroadcastControllerDelegate, IGLiveBroadcastControllerRecordingDelegate, IGLiveBroadcastLiveRoomManagerDelegate, IGLiveBroadcastResourceViewControllerDelegate, IGLiveBroadcastTutorialViewControllerDelegate, IGLiveBroadcastViewDelegate, IGLiveBroadcastViewerInviteManagerDelegate, IGLiveFeedbackControllerDelegate, IGLiveShoppingPinnedProductServiceDelegate, IGLiveAddonConforming, IGLiveRoomInviteSheetControllerDelegate, IGLiveRoomPrivacyConfirmationViewControllerDelegate, IGStoryViewerSectionControllerViewerDelegate, IGLiveRightsManagementWarningViewControllerDelegate, IGLiveInstantDemonetizationWarningViewControllerDelegate, IGLiveInviteModeratorViewControllerDelegate, IGLiveModeratorRTServiceDelegate>
{
    NSString *_broadcastId;
    IGLiveDarkOverlayView *_darkOverlayView;
    _Bool _isPolicyViolatingStream;
    _Bool _isCheckpointedForPolicyViolation;
    IGLiveRightsManagerService *_rightsManagerService;
    _Bool _isFirstLayoutPass;
    _Bool _hasShownInitialWarningForCopyrightedMusic;
    IGViewController *_sourceViewController;
    NSString *_module;
    IGLiveAddonCoordinator *_addonCoordinator;
    IGLiveViewStatusTracker *_viewStatusTracker;
    id <IGAlertHUDHandling> _alertHandler;
    IGLiveDebugInfoVideoProcessor *_debugVideoProcessor;
    IGLiveFeedbackController *_feedbackController;
    IGKeyboardObserver *_keyboardObserver;
    IGLiveBroadcastActionSheetController *_actionSheetController;
    IGLiveCommentViewUpdater *_commentViewUpdater;
    IGLiveViewerListViewController *_viewerListViewController;
    IGLiveBroadcastViewerInviteManager *_viewerInviteManager;
    IGLiveBroadcastLiveRoomManager *_liveRoomManager;
    IGLiveRoomHostBroadcastViewController *_liveRoomHostViewController;
    IGLiveBroadcastEventController *_liveRoomEventController;
    IGLiveRoomInviteSheetController *_liveRoomInviteSheetController;
    _Bool _didHaveJoinedGuests;
    IGLiveBroadcastView *_contentView;
    IGCameraViewController *_cameraViewController;
    struct CGRect _cameraWindowFrameReference;
    IGUserSession *_userSession;
    IGLiveBroadcastLogger *_logger;
    _Bool _isVisible;
    IGLiveBroadcastResourceViewController *_liveResourceViewController;
    CDStruct_eef638a9 _liveBroadcastConfig;
    NSString *_currentLiveMediaId;
    NSString *_broadcastTitle;
    NSArray *_taggedPartners;
    _Bool _isShowingAREffectTray;
    _Bool _isAnimatingAREffectTray;
    IGLiveWaveService *_waveService;
    IGBloksAsyncActionHandler *_shoppingPinProductAsyncActionHandler;
    IGBloksAsyncActionHandler *_shoppingBrandInsightsAsyncActionHandler;
    IGStoryConfettiAnimationController *_confettiAnimationController;
    IGUser *_fundraiserCharity;
    IGLiveDonationsDataSource *_donationsDataSource;
    IGLiveDonationWaveManager *_donationWaveManager;
    IGLiveUserPaySupporterListViewController *_userPaySupportersViewController;
    IGLiveUserPaySupporterDataSource *_supportersDataSource;
    _Bool _badgesEligible;
    _Bool _badgesEnabled;
    IGLiveInstantDemonetizationWarningViewController *_liveInstantDemonetizationWarningViewController;
    _Bool _hasShownInstantDemonetizationWarningSheet;
    IGLiveBroadcastSetupShoppingConfig *_shoppingConfig;
    IGLiveShoppingPinnedProductService *_pinnedProductService;
    IGSessionTracker *_shoppingSessionTracker;
    IGLiveBroadcastController *_broadcastController;
    IGLiveBroadcastCaptureManager *_broadcastCaptureManager;
    IGLiveBroadcastTutorialViewController *_broadcastTutorialViewController;
    IGLiveCommentModel *_startedLiveSystemComment;
    id <FBCancelable> _startBroadcastToken;
    long long _commentsDisplayMode;
    IGLiveCommentModel *_pinnedComment;
    IGLiveBroadcastModeratorManager *_broadcastModeratorManager;
    IGLiveInviteModeratorViewController *_inviteModeratorViewController;
    IGLiveModeratorRTService *_moderatorService;
    _Bool _disabled;
    _Bool _addonLifeCycleEventEnabled;
    id <IGLiveAddonCoordinating> _coordinator;
    NSString *_identifier;
    id <IGLiveBroadcastActiveViewControllerDelegate> _delegate;
}

- (void).cxx_destruct;
@property(nonatomic) __weak id <IGLiveBroadcastActiveViewControllerDelegate> delegate; // @synthesize delegate=_delegate;
@property(nonatomic) _Bool addonLifeCycleEventEnabled; // @synthesize addonLifeCycleEventEnabled=_addonLifeCycleEventEnabled;
@property(nonatomic) _Bool disabled; // @synthesize disabled=_disabled;
@property(readonly, nonatomic) NSString *identifier; // @synthesize identifier=_identifier;
@property(nonatomic) __weak id <IGLiveAddonCoordinating> coordinator; // @synthesize coordinator=_coordinator;
- (_Bool)accessibilityPerformEscape;
- (void)_didTapShoppingPinnedUpsell:(id)arg1;
- (void)liveInstantDemonetizationWarningViewControllerDidTapEndButton:(id)arg1;
- (void)liveRightsManagementWarningViewControllerDidTapFinishButton:(id)arg1;
- (void)storyViewerSectionController:(id)arg1 didAddSharedWithViewerSource:(id)arg2;
- (void)storyViewerSectionController:(id)arg1 didRemoveSharedWithViewerSource:(id)arg2;
- (void)storyViewerSectionController:(id)arg1 willDisplayViewerSource:(id)arg2 position:(long long)arg3;
- (void)storyViewerSectionController:(id)arg1 presentStoryForReel:(id)arg2 media:(id)arg3 fromView:(id)arg4 profilePictureView:(id)arg5;
- (void)storyViewerSectionController:(id)arg1 didHideViewerSource:(id)arg2;
- (void)storyViewerSectionController:(id)arg1 didTapProductThumbnailInViewerCell:(id)arg2;
- (void)storyViewerSectionController:(id)arg1 didTapCheckmarkButtonInViewerCell:(id)arg2;
- (void)storyViewerSectionController:(id)arg1 didTapMoreButtonInViewerCell:(id)arg2;
- (void)storyViewerSectionController:(id)arg1 didTapViewerSource:(id)arg2;
- (void)dismissResourceViewController;
- (void)rejectButtonTappedInPrivacyConfirmationViewController:(id)arg1;
- (void)acceptButtonTappedInPrivacyConfirmationViewController:(id)arg1;
- (void)liveRoomInviteSheetController:(id)arg1 didTapInviteUser:(id)arg2;
- (void)liveBroadcastLiveRoomManager:(id)arg1 willShowPrivacyConfirmationForRequestedUser:(id)arg2;
- (id)liveBroadcastLiveRoomManagerNeedShowProgressAlert:(id)arg1;
- (void)liveBroadcastLiveRoomManager:(id)arg1 didUpdateInviteBadgeCount:(long long)arg2;
- (void)liveBroadcastLiveRoomManager:(id)arg1 didReceiveFailureForInvitedUser:(id)arg2;
- (void)liveBroadcastLiveRoomManager:(id)arg1 didReceiveDeclineForInvitedUser:(id)arg2;
- (void)liveBroadcastLiveRoomManager:(id)arg1 didRemoveModerator:(id)arg2;
- (void)liveBroadcastLiveRoomManager:(id)arg1 didAddModerator:(id)arg2;
- (void)liveBroadcastLiveRoomManager:(id)arg1 didDetachInvitedUser:(id)arg2 lastUser:(_Bool)arg3;
- (void)liveBroadcastLiveRoomManager:(id)arg1 didAttachInvitedUser:(id)arg2 firstUser:(_Bool)arg3;
- (void)liveBroadcastLiveRoomManager:(id)arg1 didFinishInvitingUser:(id)arg2 cancelled:(_Bool)arg3;
- (void)liveBroadcastLiveRoomManager:(id)arg1 willBeginInvitingUsers:(id)arg2;
- (void)liveBroadcastLiveRoomManager:(id)arg1 willPresentInviteSheetForUser:(id)arg2;
- (void)liveBroadcastLiveRoomManager:(id)arg1 willPresentInviteSheetWithContext:(unsigned long long)arg2;
- (void)liveBroadcastLiveRoomManager:(id)arg1 willPresentHostSheetWithGuests:(id)arg2;
- (void)liveBroadcastViewerInviteManagerDidFinishInvitingViewer:(id)arg1;
- (void)liveBroadcastViewerInviteManagerWillBeginInvitingViewer:(id)arg1;
- (void)_showLiveRoomsTooltip;
- (void)actionSheetController:(id)arg1 didRemoveModerator:(id)arg2;
- (void)actionSheetController:(id)arg1 didAddModerator:(id)arg2;
- (void)actionSheetController:(id)arg1 didTapInviteLiveWithUser:(id)arg2;
- (void)actionSheetController:(id)arg1 didDisplayActionSheetForComment:(id)arg2 actionIdentifiers:(id)arg3;
- (void)actionSheetController:(id)arg1 didTapDeleteComment:(id)arg2;
- (void)actionSheetController:(id)arg1 didReportComment:(id)arg2;
- (void)actionSheetController:(id)arg1 didBlockUser:(id)arg2;
- (void)actionSheetController:(id)arg1 didRemoveUserFromComment:(id)arg2;
- (void)actionSheetController:(id)arg1 didHideFromUser:(id)arg2;
- (void)actionSheetController:(id)arg1 didTapUnpinComment:(id)arg2;
- (void)actionSheetController:(id)arg1 didTapPinComment:(id)arg2;
- (void)actionSheetControllerDidTapShowPracticeTutorial:(id)arg1;
- (void)actionSheetControllerDidTapDisableQuestions:(id)arg1;
- (void)actionSheetControllerDidTapAddModerator:(id)arg1;
- (void)actionSheetControllerDidTapEnableQuestions:(id)arg1;
- (void)actionSheetControllerDidTapEnableBadges:(id)arg1;
- (void)actionSheetControllerDidTapDisableBadges:(id)arg1;
- (void)actionSheetControllerDidTapDisableRequestToJoin:(id)arg1;
- (void)actionSheetControllerDidTapEnableRequestToJoin:(id)arg1;
- (void)actionSheetControllerDidTapUnmuteComment:(id)arg1;
- (void)actionSheetControllerDidTapMuteComment:(id)arg1;
- (_Bool)feedbackController:(id)arg1 updateRequestPromptIfNeeded:(id)arg2 withIncomingRequestPrompt:(id)arg3;
- (void)feedbackControllerDidFetchCommentMuteStatus:(_Bool)arg1;
- (void)feedbackController:(id)arg1 didFetchPinnedComment:(id)arg2;
- (void)feedbackControllerDidFailToUnmuteComments:(id)arg1;
- (void)feedbackControllerDidFailToMuteComments:(id)arg1;
- (void)feedbackController:(id)arg1 didFailToUnpinComment:(id)arg2;
- (void)feedbackController:(id)arg1 didFailToPinComment:(id)arg2;
- (void)feedbackController:(id)arg1 didHaveBroadcastUpdate:(id)arg2 requestTimestamp:(id)arg3;
- (void)feedbackController:(id)arg1 updateWithFloatingLikeAnimationModel:(id)arg2;
- (void)feedbackController:(id)arg1 updateWithLikeAnimationModel:(id)arg2;
- (void)feedbackController:(id)arg1 didReplaceCommentWithPk:(id)arg2 withComment:(id)arg3;
- (void)feedbackController:(id)arg1 didRemoveComment:(id)arg2;
- (void)feedbackController:(id)arg1 hasCommentsReadyToDisplay:(id)arg2 shouldLogImpression:(_Bool)arg3;
- (void)broadcastView:(id)arg1 didChangeTapToHideActive:(_Bool)arg2;
- (void)broadcastViewDidRequestToHideTrays:(id)arg1;
- (void)broadcastViewDidTapLiveResourceTooltip:(id)arg1;
- (void)broadcastViewDidTapDismissGuestButton:(id)arg1;
- (void)broadcastView:(id)arg1 didPan:(id)arg2;
- (void)broadcastViewDidTapCancelCallButton:(id)arg1;
- (void)broadcastViewDidExpandViewerList:(id)arg1;
- (void)closeButtonTappedInBroadcastView:(id)arg1;
- (void)headerTappedInBroadcastView:(id)arg1;
- (void)userListButtonTappedInBroadcastView:(id)arg1;
- (void)_broadcastViewDidTapFundraiserButton:(id)arg1;
- (void)broadcastViewDidChangeTopBannerVisibility:(id)arg1;
- (void)broadcastViewDidToggleCommentsHeight:(id)arg1 method:(id)arg2;
- (void)broadcastView:(id)arg1 didTapPinnedViewTextWithViewModel:(id)arg2 text:(id)arg3 URL:(id)arg4;
- (void)broadcastView:(id)arg1 didTapPinnedViewWithViewModel:(id)arg2;
- (void)broadcastView:(id)arg1 didTapPinnedButtonWithViewModel:(id)arg2;
- (void)broadcastView:(id)arg1 didChangeZoomScaleWithDelta:(double)arg2;
- (void)broadcastView:(id)arg1 didReceiveTapOnComment:(id)arg2 isPinned:(_Bool)arg3 shouldExpandViewerList:(_Bool)arg4 cell:(id)arg5;
- (void)_showinviteModeratorViewControllerWithLoggingMethod:(id)arg1;
- (void)broadcastView:(id)arg1 didReceiveUnpinTapOnComment:(id)arg2;
- (void)broadcastView:(id)arg1 didEnterCommentText:(id)arg2;
- (void)broadcastViewDidUnpinCurrentComment:(id)arg1;
- (_Bool)broadcastViewCanHideComments:(id)arg1;
- (_Bool)broadcastViewCanToggleCommentsHeight:(id)arg1;
- (void)broadcastView:(id)arg1 didTapWithGestureRecognizer:(id)arg2;
- (void)broadcastTutorialViewControllerDismiss:(id)arg1;
- (void)keyboardObserver:(id)arg1 keyboardFrameDidChangeWithBeginFrame:(struct CGRect)arg2 endFrame:(struct CGRect)arg3 duration:(double)arg4 curve:(long long)arg5;
- (void)keyboardObserver:(id)arg1 keyboardFrameWillChangeWithBeginFrame:(struct CGRect)arg2 endFrame:(struct CGRect)arg3 duration:(double)arg4 curve:(long long)arg5;
- (void)_displayFailToast:(id)arg1;
- (void)_sendLiveBroadcastEvent:(long long)arg1 broadcastId:(id)arg2 guestId:(id)arg3 networker:(id)arg4 currentOffsetFromVideoStart:(double)arg5 logger:(id)arg6;
- (void)_hideDarkOverlay;
- (void)_presentLiveSupportersSheet;
- (void)_presentInstantDemonetizationWarningSheet;
- (void)_presentInviteSheet:(id)arg1;
- (void)_presentViewerList;
- (void)_hideAllOpenModals;
- (void)_didRemoveModerator:(id)arg1;
- (void)_didAddModerator:(id)arg1;
- (void)liveInviteModeratorViewController:(id)arg1 didAddModerator:(id)arg2;
- (void)liveBroadcastController:(id)arg1 didEndSession:(id)arg2 diskRecordingResults:(id)arg3;
- (void)liveBroadcastControllerVideoCaptureStalled:(id)arg1;
- (void)liveBroadcastControllerAudioCaptureStalled:(id)arg1;
- (void)liveBroadcastControllerDidDismissFromCall:(id)arg1 reason:(long long)arg2;
- (void)liveBroadcastController:(id)arg1 didFailInvitationWithUserIds:(id)arg2;
- (void)liveBroadcastController:(id)arg1 guestDidDeclineInvitation:(id)arg2;
- (void)liveBroadcastControllerDidReceiveInvitation:(id)arg1;
- (void)liveBroadcastController:(id)arg1 didUpdateStatusToConnectingWithUserId:(id)arg2;
- (void)liveBroadcastLiveRoomManagerNeedsToMarkRequestsAsSeen:(id)arg1;
- (void)liveBroadcastController:(id)arg1 didRemoveVideoWithGuestIdentifier:(id)arg2;
- (void)liveBroadcastController:(id)arg1 didAddVideoInView:(id)arg2 withGuestIdentifier:(id)arg3;
- (void)liveBroadcastController:(id)arg1 broadcastForSession:(id)arg2 didFailWithError:(id)arg3;
- (void)liveBroadcastControllerDidReconnect:(id)arg1;
- (void)liveBroadcastControllerDidLoseConnection:(id)arg1;
- (void)liveBroadcastController:(id)arg1 didResumeVideoWithGuestIdentifier:(id)arg2;
- (void)liveBroadcastController:(id)arg1 didStallVideoWithGuestIdentifier:(id)arg2;
- (void)liveBroadcastControllerDidSwapBackToRtmp:(id)arg1;
- (void)liveBroadcastController:(id)arg1 didUpdateBroadcastWithStats:(id)arg2;
- (void)liveBroadcastController:(id)arg1 didConnectToServerForSession:(id)arg2 withSpeedTestSuccess:(_Bool)arg3 badgesEnabled:(_Bool)arg4;
- (void)_showShoppingTooltip;
- (id)_shoppingConfigMerchantId;
- (id)_shoppingWaterfallId;
- (void)_presentShoppingBrandInsights;
- (void)_presentShoppingPinnedProductPicker;
- (void)_handleBackOrCloseButtonTap;
- (void)_stopLiveWithBroadcast;
- (void)_startLiveWithBroadcast;
- (void)_didBlockUser:(id)arg1 success:(_Bool)arg2 error:(id)arg3;
- (void)_hideBroadcastFromUser:(id)arg1;
- (void)_showOverlayForNetworkIssue;
- (void)_displayBroadcastPauseOverlayForReason:(long long)arg1;
- (void)_resumeBroadcastIfPossible;
- (void)_pauseBroadcastWithReason:(long long)arg1;
- (void)_endBroadcastSessionWithReason:(long long)arg1 isPolicyViolation:(_Bool)arg2 policyViolationReason:(id)arg3 error:(id)arg4;
- (void)_endFailedBroadcastSessionWithErrorMessage:(id)arg1 error:(id)arg2;
- (void)_endSuccessfulBroadcastSessionWithReason:(long long)arg1 isPolicyViolation:(_Bool)arg2 policyViolationReason:(id)arg3;
- (void)_broadcastDidStart;
- (void)_displayNoViewersSystemCommentIfNeeded;
- (void)_displayStartupSystemComment;
- (void)_configureViewWithShoppingNullState;
- (id)broadcastController;
- (_Bool)_isLiveResourceShowing;
- (void)_presentLiveBroadcastTutorialViewController;
- (void)_presentLiveResourceViewController;
- (void)_configureLiveResourceViewControllerWithType:(id)arg1 resourceData:(id)arg2;
- (_Bool)_isViewerListShowing;
- (void)_handleSoftPolicyViolationReason:(id)arg1;
- (void)_pinComment:(id)arg1;
- (_Bool)_isCommentFromGuest:(id)arg1;
- (void)shoppingPinnedProductService:(id)arg1 didRequestPresentCheckoutWithViewModel:(id)arg2;
- (void)shoppingPinnedProductService:(id)arg1 didLaunchPinnedProductWithViewModel:(id)arg2;
- (void)shoppingPinnedProductServiceDidRequestPresentShoppingEntrypoint:(id)arg1;
- (void)shoppingPinnedProductService:(id)arg1 didRequestPresentShoppingCartWithMerchantPk:(id)arg2;
- (void)shoppingPinnedProductService:(id)arg1 didRequestPresentPinnedProductPickerWithViewModel:(id)arg2;
- (void)shoppingPinnedProductService:(id)arg1 didReceivePinnedProductEvent:(id)arg2;
- (void)shoppingPinnedProductService:(id)arg1 didUpdatePinnedProductWithViewModel:(id)arg2 previousViewModel:(id)arg3;
- (void)liveModeratorRTService:(id)arg1 didReceiveModeratorUpdate:(id)arg2;
- (void)reloadCameraEffectTrayData;
- (void)hideAREffectInstructions;
- (void)showAREffectInstructions:(id)arg1 forDuration:(double)arg2;
- (void)showAREffectInstructions:(id)arg1;
- (id)handleRequest:(id)arg1;
- (_Bool)canHandleRequest:(id)arg1;
- (void)receiveEvent:(unsigned long long)arg1 from:(id)arg2 payload:(id)arg3;
- (void)_onAppWillTerminate:(id)arg1;
- (void)_onAppDidBecomeActive:(id)arg1;
- (void)_onAppWillResignActive:(id)arg1;
- (_Bool)prefersStatusBarHidden;
- (void)viewDidLayoutSubviews;
- (void)viewDidLoad;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)viewWillDisappear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewWillAppear:(_Bool)arg1;
@property(readonly, nonatomic) UIView *cameraGesturesContainerView;
- (double)_safeAreaCornerRadius;
- (void)updateCameraViewControllerFrame:(struct CGRect)arg1;
- (void)prepareCameraViewControllerFrame:(struct CGRect)arg1;
- (void)setCameraViewController:(id)arg1;
- (void)setStartingCameraPosition:(long long)arg1;
- (void)dealloc;
- (id)initWithUserSession:(id)arg1 logger:(id)arg2 broadcastToolbox:(id)arg3 liveBroadcastConfig:(CDStruct_eef638a9)arg4 fundraiserCharity:(id)arg5 shoppingConfig:(id)arg6 broadcastTitle:(id)arg7 taggedPartners:(id)arg8 sourceViewController:(id)arg9 broadcastModeratorManager:(id)arg10;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

