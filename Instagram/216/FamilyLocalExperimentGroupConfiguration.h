//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class NSString;

@interface FamilyLocalExperimentGroupConfiguration : NSObject
{
    NSString *_groupName;
    unsigned long long _numberOfAllocatedSegments;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) unsigned long long numberOfAllocatedSegments; // @synthesize numberOfAllocatedSegments=_numberOfAllocatedSegments;
@property(readonly, nonatomic) NSString *groupName; // @synthesize groupName=_groupName;
- (id)initWithGroupName:(id)arg1 numberOfAllocatedSegments:(unsigned long long)arg2;

@end

