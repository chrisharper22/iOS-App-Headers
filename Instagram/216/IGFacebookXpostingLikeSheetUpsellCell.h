//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <UIKit/UICollectionViewCell.h>

@class IGTapButton, IGTextButton, UIActivityIndicatorView, UIImageView, UILabel, UIView;
@protocol IGFacebookXpostingLikeSheetUpsellCellDelegate;

@interface IGFacebookXpostingLikeSheetUpsellCell : UICollectionViewCell
{
    UIView *_iconContainer;
    UIImageView *_fbIcon;
    UILabel *_title;
    IGTextButton *_shareButton;
    IGTapButton *_closeButton;
    id <IGFacebookXpostingLikeSheetUpsellCellDelegate> _delegate;
    UIActivityIndicatorView *_spinner;
}

- (void).cxx_destruct;
- (void)_startAnimatingSpinner;
- (void)_didTapShareButton;
- (void)_didTapCloseButton;
- (void)layoutSubviews;
- (void)configureCellWithDelegate:(id)arg1;
- (id)initWithFrame:(struct CGRect)arg1;

@end

