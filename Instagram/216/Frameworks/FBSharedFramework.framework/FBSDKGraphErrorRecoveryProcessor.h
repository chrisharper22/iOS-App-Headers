//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class FBSDKErrorRecoveryAttempter, NSError, NSString;
@protocol FBSDKGraphErrorRecoveryProcessorDelegate;

@interface FBSDKGraphErrorRecoveryProcessor : NSObject
{
    NSString *_accessToken;
    id <FBSDKGraphErrorRecoveryProcessorDelegate> _delegate;
    FBSDKErrorRecoveryAttempter *_recoveryAttempter;
    NSError *__error;
}

+ (id)new;
- (void).cxx_destruct;
@property(retain, nonatomic) NSError *_error; // @synthesize _error=__error;
@property(retain, nonatomic) FBSDKErrorRecoveryAttempter *recoveryAttempter; // @synthesize recoveryAttempter=_recoveryAttempter;
@property(readonly, nonatomic) __weak id <FBSDKGraphErrorRecoveryProcessorDelegate> delegate; // @synthesize delegate=_delegate;
@property(readonly, nonatomic) NSString *accessToken; // @synthesize accessToken=_accessToken;
- (_Bool)processError:(id)arg1 request:(id)arg2 delegate:(id)arg3;
- (id)initWithAccessTokenString:(id)arg1;
- (id)init;

@end

