//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <Foundation/NSCoder.h>

@class NSData;

@interface FBMessagePackCoder : NSCoder
{
    struct vector<FBMPCEncodingObjectContext *, std::allocator<FBMPCEncodingObjectContext *>> _encodingContextStack;
    struct vector<FBMPCDecodingObjectContext, std::allocator<FBMPCDecodingObjectContext>> _decodingObjectStack;
    struct MessagePackParseState _decodingParseState;
    id _rootObject;
    struct FBMPCEncodingObjectContext _encodingRootContext;
    unsigned int _systemVersion;
    _Bool _useParseState;
    NSData *_data;
}

+ (id)unpackObjectWithData:(id)arg1 error:(id *)arg2;
+ (id)unpackObjectWithData:(id)arg1;
+ (id)packedDataWithRootObject:(id)arg1 error:(id *)arg2;
+ (id)packedDataWithRootObject:(id)arg1;
- (id).cxx_construct;
- (void).cxx_destruct;
@property(copy, nonatomic) NSData *data; // @synthesize data=_data;
- (id)unpackObjectWithData:(id)arg1 error:(id *)arg2;
- (id)unpackObjectWithData:(id)arg1;
- (id)packedDataWithRootObject:(id)arg1;
- (long long)versionForClassName:(id)arg1;
- (unsigned int)systemVersion;
- (void)_prepareForReuse;
- (void)finishEncoding;
- (int)decodeIntForKey:(id)arg1;
- (long long)decodeIntegerForKey:(id)arg1;
- (long long)decodeInt64ForKey:(id)arg1;
- (int)decodeInt32ForKey:(id)arg1;
- (float)decodeFloatForKey:(id)arg1;
- (double)decodeDoubleForKey:(id)arg1;
- (_Bool)decodeBoolForKey:(id)arg1;
- (id)decodeObjectForKey:(id)arg1;
- (void)encodeObject:(id)arg1 forKey:(id)arg2;
- (void)encodeRootObject:(id)arg1;
- (void)encodeInt64:(long long)arg1 forKey:(id)arg2;
- (void)encodeInt32:(int)arg1 forKey:(id)arg2;
- (void)encodeInteger:(long long)arg1 forKey:(id)arg2;
- (void)encodeInt:(int)arg1 forKey:(id)arg2;
- (void)encodeFloat:(float)arg1 forKey:(id)arg2;
- (void)encodeDouble:(double)arg1 forKey:(id)arg2;
- (void)encodeBool:(_Bool)arg1 forKey:(id)arg2;
- (void)decodeValueOfObjCType:(const char *)arg1 at:(void *)arg2;
- (const char *)decodeBytesForKey:(id)arg1 returnedLength:(unsigned long long *)arg2;
- (void *)decodeBytesWithReturnedLength:(unsigned long long *)arg1;
- (id)decodeDataObject;
- (_Bool)containsValueForKey:(id)arg1;
- (void)encodeDataObject:(id)arg1;
- (void)encodeBytesPtr:(unique_ptr_ea0661fb)arg1 length:(unsigned long long)arg2 forKey:(id)arg3;
- (void)encodeBytes:(const char *)arg1 length:(unsigned long long)arg2 forKey:(id)arg3;
- (void)encodeBytes:(const void *)arg1 length:(unsigned long long)arg2;
- (void)encodeValueOfObjCType:(const char *)arg1 at:(const void *)arg2;
- (void)dealloc;
- (id)init;
- (id)initWithUseParseState:(_Bool)arg1;
- (_Bool)allowsKeyedCoding;

@end

