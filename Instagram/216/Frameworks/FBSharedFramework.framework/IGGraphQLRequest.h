//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import <FBSharedFramework/NSCopying-Protocol.h>

@class NSDictionary, NSString;
@protocol IGGraphQLCachePolicy, IGGraphQLParsing;

@interface IGGraphQLRequest : NSObject <NSCopying>
{
    _Bool _useDjangoProxy;
    _Bool _ignoreNonCriticalErrors;
    _Bool _doNotCacheResponse;
    unsigned int _fieldSetId;
    NSString *_queryId;
    NSDictionary *_queryParams;
    id <IGGraphQLParsing> _parser;
    unsigned long long _operationType;
    unsigned long long _requestType;
    unsigned long long _schemaType;
    id <IGGraphQLCachePolicy> _cachePolicy;
    NSString *_vcPolicy;
    unsigned long long _cacheTtlSeconds;
    CDUnknownBlockType _successHandler;
    CDUnknownBlockType _failHandler;
}

- (void).cxx_destruct;
@property(readonly, copy, nonatomic) CDUnknownBlockType failHandler; // @synthesize failHandler=_failHandler;
@property(readonly, copy, nonatomic) CDUnknownBlockType successHandler; // @synthesize successHandler=_successHandler;
@property(nonatomic) _Bool doNotCacheResponse; // @synthesize doNotCacheResponse=_doNotCacheResponse;
@property(readonly, nonatomic) unsigned long long cacheTtlSeconds; // @synthesize cacheTtlSeconds=_cacheTtlSeconds;
@property(readonly, copy, nonatomic) NSString *vcPolicy; // @synthesize vcPolicy=_vcPolicy;
@property(readonly, nonatomic) id <IGGraphQLCachePolicy> cachePolicy; // @synthesize cachePolicy=_cachePolicy;
@property(readonly, nonatomic) _Bool ignoreNonCriticalErrors; // @synthesize ignoreNonCriticalErrors=_ignoreNonCriticalErrors;
@property(readonly, nonatomic) _Bool useDjangoProxy; // @synthesize useDjangoProxy=_useDjangoProxy;
@property(readonly, nonatomic) unsigned long long schemaType; // @synthesize schemaType=_schemaType;
@property(readonly, nonatomic) unsigned long long requestType; // @synthesize requestType=_requestType;
@property(readonly, nonatomic) unsigned long long operationType; // @synthesize operationType=_operationType;
@property(readonly, nonatomic) unsigned int fieldSetId; // @synthesize fieldSetId=_fieldSetId;
@property(readonly, nonatomic) id <IGGraphQLParsing> parser; // @synthesize parser=_parser;
@property(readonly, copy, nonatomic) NSDictionary *queryParams; // @synthesize queryParams=_queryParams;
@property(readonly, copy, nonatomic) NSString *queryId; // @synthesize queryId=_queryId;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (unsigned long long)hash;
- (_Bool)isEqual:(id)arg1;
- (_Bool)_isEqualToRequest:(id)arg1;
- (_Bool)isCacheValidForObject:(id)arg1;
- (void)successWithResult:(id)arg1 statusSource:(long long)arg2;
- (void)failWithError:(id)arg1;
- (id)initRequestWithTreeShapeId:(id)arg1 fieldSetId:(unsigned int)arg2 queryParams:(id)arg3 operationType:(unsigned long long)arg4 requestType:(unsigned long long)arg5 schemaType:(unsigned long long)arg6 useDjangoProxy:(_Bool)arg7 ignoreNonCriticalErrors:(_Bool)arg8 vcPolicy:(id)arg9 parser:(id)arg10 cachePolicy:(id)arg11 cacheTtlSeconds:(unsigned long long)arg12 successHandler:(CDUnknownBlockType)arg13 failHandler:(CDUnknownBlockType)arg14;
- (id)initRequestWithQueryId:(id)arg1 queryParams:(id)arg2 requestType:(unsigned long long)arg3 schemaType:(unsigned long long)arg4 useDjangoProxy:(_Bool)arg5 ignoreNonCriticalErrors:(_Bool)arg6 vcPolicy:(id)arg7 parser:(id)arg8 cachePolicy:(id)arg9 successHandler:(CDUnknownBlockType)arg10 failHandler:(CDUnknownBlockType)arg11;

@end

