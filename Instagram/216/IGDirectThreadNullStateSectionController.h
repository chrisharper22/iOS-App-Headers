//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <FBSharedFramework/IGListBindingSingleSectionController.h>

#import "IGDirectNullStateCellDelegate-Protocol.h"
#import "IGListDisplayDelegate-Protocol.h"

@class IGDirectMessageSectionComponents, IGDirectThreadNullStateViewModel, IGDirectThreadScopedNUXProvider, IGLoggerThreadInfo, IGUserSession, NSString;

@interface IGDirectThreadNullStateSectionController : IGListBindingSingleSectionController <IGDirectNullStateCellDelegate, IGListDisplayDelegate>
{
    IGDirectThreadNullStateViewModel *_viewModel;
    IGDirectMessageSectionComponents *_components;
    IGDirectThreadScopedNUXProvider *_nuxProvider;
    IGLoggerThreadInfo *_loggerThreadInfo;
    IGUserSession *_userSession;
}

- (void).cxx_destruct;
- (void)nullStateButtonWasPressed:(id)arg1 threadType:(id)arg2;
- (void)listAdapter:(id)arg1 didEndDisplayingSectionController:(id)arg2 cell:(id)arg3 atIndex:(long long)arg4;
- (void)listAdapter:(id)arg1 willDisplaySectionController:(id)arg2 cell:(id)arg3 atIndex:(long long)arg4;
- (void)listAdapter:(id)arg1 didEndDisplayingSectionController:(id)arg2;
- (void)listAdapter:(id)arg1 willDisplaySectionController:(id)arg2;
- (void)didSelectItemAtIndex:(long long)arg1;
- (void)didUpdateToObject:(id)arg1;
- (void)configureCell:(id)arg1 withViewModel:(id)arg2;
- (Class)cellClass;
- (struct CGSize)sizeForViewModel:(id)arg1;
- (id)initWithNuxProvider:(id)arg1 components:(id)arg2 loggerThreadInfo:(id)arg3 userSession:(id)arg4;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

