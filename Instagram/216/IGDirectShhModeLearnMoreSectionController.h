//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <FBSharedFramework/IGListBindingSingleSectionController.h>

#import "IGDirectCoreTextViewCellDelegate-Protocol.h"
#import "IGListDisplayDelegate-Protocol.h"

@class IGDirectShhModeLearnMoreViewModel, IGDirectThreadScopedNUXProvider, IGUserSession, NSString;

@interface IGDirectShhModeLearnMoreSectionController : IGListBindingSingleSectionController <IGListDisplayDelegate, IGDirectCoreTextViewCellDelegate>
{
    IGDirectShhModeLearnMoreViewModel *_viewModel;
    IGDirectThreadScopedNUXProvider *_nuxProvider;
    IGUserSession *_userSession;
}

- (void).cxx_destruct;
- (void)messageListCellDidTapLink:(id)arg1;
- (void)listAdapter:(id)arg1 didEndDisplayingSectionController:(id)arg2 cell:(id)arg3 atIndex:(long long)arg4;
- (void)listAdapter:(id)arg1 willDisplaySectionController:(id)arg2 cell:(id)arg3 atIndex:(long long)arg4;
- (void)listAdapter:(id)arg1 didEndDisplayingSectionController:(id)arg2;
- (void)listAdapter:(id)arg1 willDisplaySectionController:(id)arg2;
- (void)didUpdateToObject:(id)arg1;
- (struct CGSize)sizeForViewModel:(id)arg1;
- (void)configureCell:(id)arg1 withViewModel:(id)arg2;
- (Class)cellClass;
- (id)initWithThreadScopedNUXProvider:(id)arg1 userSession:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

