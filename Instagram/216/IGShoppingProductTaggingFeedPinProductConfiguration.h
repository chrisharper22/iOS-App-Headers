//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class IGVideoComposition, NSString;

@interface IGShoppingProductTaggingFeedPinProductConfiguration : NSObject
{
    long long _surfaceType;
    IGVideoComposition *_videoComposition;
    NSString *_mediaPk;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) NSString *mediaPk; // @synthesize mediaPk=_mediaPk;
@property(readonly, nonatomic) IGVideoComposition *videoComposition; // @synthesize videoComposition=_videoComposition;
- (_Bool)productPinningEnabledWithTaggedProducts:(id)arg1;
- (_Bool)isEligibleForProductPinning;
- (id)initWithSurfaceType:(long long)arg1 videoComposition:(id)arg2 mediaPk:(id)arg3;

@end

