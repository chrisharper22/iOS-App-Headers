//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <FBSharedFramework/IGViewController.h>

#import "IGCropViewUserInteractionDelegate-Protocol.h"
#import "IGEditorTabBarDelegate-Protocol.h"
#import "IGFBFilterCollectionControllerDelegate-Protocol.h"
#import "IGFeedDraftManagerEventListener-Protocol.h"
#import "IGFeedGalleryAssetFetchRequestDelegate-Protocol.h"
#import "IGFilterCollectionControllerDelegate-Protocol.h"
#import "IGFilterControlViewDelegate-Protocol.h"
#import "IGMediaLibraryViewControllerDelegate-Protocol.h"
#import "IGShareViewControllerDelegate-Protocol.h"
#import "IGSoundStateListenerDelegate-Protocol.h"
#import "IGVideoPlaybackViewDelegate-Protocol.h"
#import "IGVideoScrubberViewDelegate-Protocol.h"
#import "IGVideoTrimConfirmationDelegate-Protocol.h"
#import "IGVideoTrimControllerDelegate-Protocol.h"

@class IGAudioSessionClient, IGCropView, IGEditorAssetLoadingView, IGEditorTabBar, IGEditorTrayAccessibilityElement, IGFBFilterCollectionController, IGFBVideoPlaybackView, IGFeedCreationFlowLogger, IGFeedEditorNavigationPerfComponents, IGFilterCollectionController, IGFilterControlView, IGIGLContext, IGMediaMetadata, IGSegmentedProgressView, IGShareListManager, IGSoundStateListener, IGTooltipView, IGUserSession, IGVideoComposition, IGVideoInfo, IGVideoPlayButton, IGVideoPlaybackView, IGVideoScrubberView, IGVideoTrimViewController, NSMutableDictionary, NSString, UIBarButtonItem, UIButton, UICollectionView, UIImage, UIImageView, UILabel, UIView;
@protocol IGEditorTabView, IGFeedGalleryAssetFetchRequest, IGVideoEditorDelegate, IGVideoPlaybackViewType;

@interface IGVideoEditorViewController : IGViewController <IGCropViewUserInteractionDelegate, IGFilterCollectionControllerDelegate, IGFBFilterCollectionControllerDelegate, IGVideoPlaybackViewDelegate, IGEditorTabBarDelegate, IGVideoScrubberViewDelegate, IGFeedGalleryAssetFetchRequestDelegate, IGFilterControlViewDelegate, IGVideoTrimControllerDelegate, IGVideoTrimConfirmationDelegate, IGSoundStateListenerDelegate, IGMediaLibraryViewControllerDelegate, IGFeedDraftManagerEventListener, IGShareViewControllerDelegate>
{
    IGCropView *_cropView;
    _Bool _editorCropEnabled;
    _Bool _isNewGalleryFlow;
    id <IGFeedGalleryAssetFetchRequest> _galleryFetchRequest;
    UIImage *_loadingPlaceholderImage;
    UIImageView *_loadingPlaceholderImageView;
    IGEditorAssetLoadingView *_assetLoadingView;
    IGFeedCreationFlowLogger *_logger;
    IGFeedEditorNavigationPerfComponents *_perfComponents;
    _Bool _useFbIgluFilters;
    IGIGLContext *_iglContext;
    IGEditorTrayAccessibilityElement *_collectionAccessibilityElement;
    long long _filterTrayCurrentIndex;
    _Bool _needsVideoUpdate;
    _Bool _trimAnimationAlreadyShown;
    _Bool _isClipTrimMode;
    _Bool _shouldMuteAudio;
    _Bool _didReorderTray;
    _Bool _videoIsReady;
    _Bool _hasCancelledShareVC;
    id <IGVideoEditorDelegate> _delegate;
    IGUserSession *_userSession;
    IGVideoInfo *_videoInfo;
    IGVideoComposition *_workingVideoComposition;
    IGVideoPlaybackView *_igPlayerView;
    IGFBVideoPlaybackView *_fbPlayerView;
    IGVideoPlayButton *_playButton;
    IGMediaMetadata *_mediaMetadata;
    IGShareListManager *_shareListManager;
    NSMutableDictionary *_adjustedFilterStrengthValues;
    UIBarButtonItem *_doneButton;
    UIBarButtonItem *_cancelButton;
    UIImageView *_cinemaButton;
    UIImageView *_audioMuteButton;
    UIImageView *_cropButton;
    UIView<IGEditorTabView> *_coverFrameButton;
    UIView<IGEditorTabView> *_trimmerButton;
    UIView<IGEditorTabView> *_filtersButton;
    UIView *_audioMuteOverlayView;
    UILabel *_audioMutedLabel;
    UILabel *_audioOnLabel;
    long long _audioMuteToggleTick;
    double _originalVideoLength;
    UIView *_toolsContainerView;
    IGEditorTabBar *_toolsTabBar;
    IGEditorTabBar *_toggleToolsTabBar;
    UIView *_toolView;
    UICollectionView *_filterCollectionView;
    IGFilterCollectionController *_igFilterTrayController;
    IGFBFilterCollectionController *_fbFilterTrayController;
    IGFilterControlView *_filterControlView;
    UIButton *_leftChevronButton;
    UIButton *_rightChevronButton;
    IGVideoTrimViewController *_videoTrimViewController;
    IGSegmentedProgressView *_trimProgressView;
    IGVideoScrubberView *_scrubberView;
    IGTooltipView *_trimTooltipView;
    long long _mediaOrigin;
    IGAudioSessionClient *_audioClient;
    IGSoundStateListener *_soundStateListener;
    struct CGSize _videoDisplaySize;
    CDStruct_1b6d18a9 _originalClipDuration;
    CDStruct_1b6d18a9 _keyTime;
    CDStruct_e83c9415 _previousTrimmedRange;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) IGSoundStateListener *soundStateListener; // @synthesize soundStateListener=_soundStateListener;
@property(readonly, nonatomic) IGAudioSessionClient *audioClient; // @synthesize audioClient=_audioClient;
@property(nonatomic) _Bool hasCancelledShareVC; // @synthesize hasCancelledShareVC=_hasCancelledShareVC;
@property(nonatomic) long long mediaOrigin; // @synthesize mediaOrigin=_mediaOrigin;
@property(nonatomic) _Bool videoIsReady; // @synthesize videoIsReady=_videoIsReady;
@property(nonatomic) _Bool didReorderTray; // @synthesize didReorderTray=_didReorderTray;
@property(nonatomic) CDStruct_1b6d18a9 keyTime; // @synthesize keyTime=_keyTime;
@property(nonatomic) CDStruct_1b6d18a9 originalClipDuration; // @synthesize originalClipDuration=_originalClipDuration;
@property(nonatomic) _Bool shouldMuteAudio; // @synthesize shouldMuteAudio=_shouldMuteAudio;
@property(nonatomic) _Bool isClipTrimMode; // @synthesize isClipTrimMode=_isClipTrimMode;
@property(nonatomic) _Bool trimAnimationAlreadyShown; // @synthesize trimAnimationAlreadyShown=_trimAnimationAlreadyShown;
@property(nonatomic) _Bool needsVideoUpdate; // @synthesize needsVideoUpdate=_needsVideoUpdate;
@property(retain, nonatomic) IGTooltipView *trimTooltipView; // @synthesize trimTooltipView=_trimTooltipView;
@property(retain, nonatomic) IGVideoScrubberView *scrubberView; // @synthesize scrubberView=_scrubberView;
@property(retain, nonatomic) IGSegmentedProgressView *trimProgressView; // @synthesize trimProgressView=_trimProgressView;
@property(nonatomic) CDStruct_e83c9415 previousTrimmedRange; // @synthesize previousTrimmedRange=_previousTrimmedRange;
@property(retain, nonatomic) IGVideoTrimViewController *videoTrimViewController; // @synthesize videoTrimViewController=_videoTrimViewController;
@property(retain, nonatomic) UIButton *rightChevronButton; // @synthesize rightChevronButton=_rightChevronButton;
@property(retain, nonatomic) UIButton *leftChevronButton; // @synthesize leftChevronButton=_leftChevronButton;
@property(retain, nonatomic) IGFilterControlView *filterControlView; // @synthesize filterControlView=_filterControlView;
@property(retain, nonatomic) IGFBFilterCollectionController *fbFilterTrayController; // @synthesize fbFilterTrayController=_fbFilterTrayController;
@property(retain, nonatomic) IGFilterCollectionController *igFilterTrayController; // @synthesize igFilterTrayController=_igFilterTrayController;
@property(retain, nonatomic) UICollectionView *filterCollectionView; // @synthesize filterCollectionView=_filterCollectionView;
@property(retain, nonatomic) UIView *toolView; // @synthesize toolView=_toolView;
@property(retain, nonatomic) IGEditorTabBar *toggleToolsTabBar; // @synthesize toggleToolsTabBar=_toggleToolsTabBar;
@property(retain, nonatomic) IGEditorTabBar *toolsTabBar; // @synthesize toolsTabBar=_toolsTabBar;
@property(retain, nonatomic) UIView *toolsContainerView; // @synthesize toolsContainerView=_toolsContainerView;
@property(nonatomic) double originalVideoLength; // @synthesize originalVideoLength=_originalVideoLength;
@property(nonatomic) long long audioMuteToggleTick; // @synthesize audioMuteToggleTick=_audioMuteToggleTick;
@property(retain, nonatomic) UILabel *audioOnLabel; // @synthesize audioOnLabel=_audioOnLabel;
@property(retain, nonatomic) UILabel *audioMutedLabel; // @synthesize audioMutedLabel=_audioMutedLabel;
@property(retain, nonatomic) UIView *audioMuteOverlayView; // @synthesize audioMuteOverlayView=_audioMuteOverlayView;
@property(retain, nonatomic) UIView<IGEditorTabView> *filtersButton; // @synthesize filtersButton=_filtersButton;
@property(retain, nonatomic) UIView<IGEditorTabView> *trimmerButton; // @synthesize trimmerButton=_trimmerButton;
@property(retain, nonatomic) UIView<IGEditorTabView> *coverFrameButton; // @synthesize coverFrameButton=_coverFrameButton;
@property(retain, nonatomic) UIImageView *cropButton; // @synthesize cropButton=_cropButton;
@property(retain, nonatomic) UIImageView *audioMuteButton; // @synthesize audioMuteButton=_audioMuteButton;
@property(retain, nonatomic) UIImageView *cinemaButton; // @synthesize cinemaButton=_cinemaButton;
@property(retain, nonatomic) UIBarButtonItem *cancelButton; // @synthesize cancelButton=_cancelButton;
@property(retain, nonatomic) UIBarButtonItem *doneButton; // @synthesize doneButton=_doneButton;
@property(retain, nonatomic) NSMutableDictionary *adjustedFilterStrengthValues; // @synthesize adjustedFilterStrengthValues=_adjustedFilterStrengthValues;
@property(retain, nonatomic) IGShareListManager *shareListManager; // @synthesize shareListManager=_shareListManager;
@property(retain, nonatomic) IGMediaMetadata *mediaMetadata; // @synthesize mediaMetadata=_mediaMetadata;
@property(retain, nonatomic) IGVideoPlayButton *playButton; // @synthesize playButton=_playButton;
@property(retain, nonatomic) IGFBVideoPlaybackView *fbPlayerView; // @synthesize fbPlayerView=_fbPlayerView;
@property(retain, nonatomic) IGVideoPlaybackView *igPlayerView; // @synthesize igPlayerView=_igPlayerView;
@property(retain, nonatomic) IGVideoComposition *workingVideoComposition; // @synthesize workingVideoComposition=_workingVideoComposition;
@property(retain, nonatomic) IGVideoInfo *videoInfo; // @synthesize videoInfo=_videoInfo;
@property(nonatomic) struct CGSize videoDisplaySize; // @synthesize videoDisplaySize=_videoDisplaySize;
@property(readonly, nonatomic) IGUserSession *userSession; // @synthesize userSession=_userSession;
@property(nonatomic) __weak id <IGVideoEditorDelegate> delegate; // @synthesize delegate=_delegate;
- (void)igFeedDraftManagerDidFailToSaveDraftWithError:(id)arg1;
- (void)igFeedDraftManagerDidSaveDraft:(id)arg1;
- (void)_resetVideoTimestamps;
- (void)_updateVideoDependentViews;
- (void)_videoInfoReady:(id)arg1;
- (void)_updateVideoDisplaySize;
- (double)_videoContentWidth;
- (void)_cancelRenderAndUpload;
- (void)soundListenerDidSetStickySoundState:(id)arg1;
- (void)soundListenerDidUpdate:(id)arg1;
- (void)videoTrimControllerDidRejectChanges:(id)arg1;
- (void)videoTrimController:(id)arg1 didConfirmTrimmedRange:(CDStruct_e83c9415)arg2;
- (void)videoTrimController:(id)arg1 didFinishUpdatingWithTrimmedRange:(CDStruct_e83c9415)arg2;
- (void)videoTrimController:(id)arg1 didUpdateWithTrimmedRange:(CDStruct_e83c9415)arg2;
- (void)videoTrimController:(id)arg1 willBeginUpdatingWithTrimmedRange:(CDStruct_e83c9415)arg2;
- (id)_cacheKeyForClip:(id)arg1 filterName:(id)arg2 time:(CDStruct_1b6d18a9)arg3;
- (id)cachedThumbnailForClip:(id)arg1 andTime:(CDStruct_1b6d18a9)arg2;
- (void)_cacheThumbnail:(id)arg1 forClip:(id)arg2 andTime:(CDStruct_1b6d18a9)arg3;
- (void)_populateThumbnailCacheForClipIndex:(unsigned long long)arg1;
- (void)_generateScrubberFrames;
- (void)scrubberViewDidTapOnHandle;
- (void)scrubberViewFinishedMovingHandle;
- (void)scrubberViewBeganMovingHandle;
- (void)scrubberViewDidMoveHandleToPosition:(double)arg1;
- (void)_hideTrimTooltip:(_Bool)arg1;
- (double)_trimViewTimeScaleForClipDuration:(double)arg1;
- (void)_showVideoTrimmer;
- (void)_showCoverFramePicker;
- (void)_showClips;
- (void)_showFilters;
- (void)_reconcileVideoPlayer;
- (void)mediaLibraryViewController:(id)arg1 didSharePanavisionComposition:(id)arg2 toDestination:(long long)arg3;
- (void)mediaLibraryViewControllerDidCancelNftMediaSelection:(id)arg1;
- (void)mediaLibraryViewController:(id)arg1 didFinishWithNftMedia:(id)arg2;
- (void)mediaLibraryViewController:(id)arg1 didFinishWithCompositionDraft:(id)arg2;
- (void)mediaLibraryViewController:(id)arg1 didFinishWithMediaDraft:(id)arg2;
- (void)mediaLibraryViewController:(id)arg1 didProduceComposition:(id)arg2;
- (void)mediaLibraryViewControllerDidCancel:(id)arg1;
- (void)_updateClipsWithVideoComposition:(id)arg1;
- (void)_exitTrimControlView;
- (void)_updateChevronStatusForScrollView:(id)arg1;
- (void)filterControllerDidScroll:(id)arg1;
- (struct CGRect)_pickerFrame;
- (void)filterControlView:(id)arg1 didChangetintIntensity:(double)arg2 tintType:(unsigned long long)arg3;
- (void)filterControlView:(id)arg1 didChangeTintColor:(unsigned long long)arg2 withTintType:(unsigned long long)arg3;
- (void)filterControlViewNeedUpdateValue;
- (void)filterControlViewDidTapRotate:(id)arg1;
- (void)filterControlView:(id)arg1 didChangeValue:(double)arg2;
- (void)filterControlViewDidEndDragging:(id)arg1;
- (void)filterControlViewDidStartDragging:(id)arg1;
- (void)filterControlViewDidPressCancel:(id)arg1;
- (void)filterControlViewDidPressDone:(id)arg1;
- (void)_onToolCancel:(id)arg1;
- (void)_onToolDone:(id)arg1;
- (void)_dismissFilterControlView:(id)arg1;
- (void)_presentFilterControlView:(id)arg1 withTitle:(id)arg2 subtitle:(id)arg3 leftBarButtonItem:(id)arg4 rightBarButtonItem:(id)arg5 andCompletion:(CDUnknownBlockType)arg6;
- (void)_presentFilterControlView:(id)arg1 withTitle:(id)arg2 subtitle:(id)arg3 andCompletion:(CDUnknownBlockType)arg4;
- (id)_titleForItem:(id)arg1;
- (void)tabBar:(id)arg1 didSelectTabView:(id)arg2 previousTabView:(id)arg3;
- (_Bool)tabBar:(id)arg1 shouldSelectTabView:(id)arg2;
- (id)_activeTrimTabView;
- (void)_showTrimOverlayNux;
- (void)_showTrimAnimation;
- (void)shareViewControllerWillFinish:(id)arg1;
- (void)shareViewControllerDidCancel:(id)arg1;
- (void)shareViewControllerDidAppear:(id)arg1;
- (void)_toggleAspectRatio;
- (void)_toggleAudioMuting;
- (_Bool)_isAudioMutingEnabled;
- (void)_setNavigationItemTitle:(id)arg1 subtitle:(id)arg2;
- (id)_createTitleView:(id)arg1 withSubtitle:(id)arg2;
- (void)_updateTrimProgressSegmentColors;
- (void)fbFilterControllerDidStartReordering:(id)arg1;
- (double)fbFilterController:(id)arg1 filterStrengthForIdentifier:(id)arg2;
- (void)fbFilterController:(id)arg1 didSelectFilterStrengthForIdentifier:(id)arg2;
- (void)fbFilterController:(id)arg1 didSelectFilterWithIdentifier:(id)arg2 changed:(_Bool)arg3 willScroll:(_Bool)arg4;
- (void)fbFilterControllerDidGenerateFilterPreview:(id)arg1;
- (void)fbFilterControllerWillGenerateFilterPreview:(id)arg1;
- (void)filterControllerDidStartReordering:(id)arg1;
- (double)filterController:(id)arg1 filterStrengthForClass:(Class)arg2;
- (void)filterController:(id)arg1 didSelectFilterStrengthForClass:(Class)arg2;
- (void)filterController:(id)arg1 didSelectFilterWithClass:(Class)arg2 changed:(_Bool)arg3 willScroll:(_Bool)arg4;
- (void)filterControllerDidGenerateFilterPreview:(id)arg1;
- (void)filterControllerWillGenerateFilterPreview:(id)arg1;
- (void)playbackView:(id)arg1 effectivePlaybackRateDidChange:(double)arg2;
- (void)playbackView:(id)arg1 didCompleteLoopWithPlaybackRate:(double)arg2;
- (void)playbackView:(id)arg1 didPlayToTime:(CDStruct_1b6d18a9)arg2;
- (void)filterPlayerViewDidFinishPlayingWithAnimation:(_Bool)arg1;
- (void)_filterPlayerViewDidFinishPlaying;
- (id)_sendAnalyticsEvent:(id)arg1;
- (void)_prepareForShareStep;
- (void)_done;
- (void)_fbInitializeFromEditsDictionary;
- (void)_igInitializeFromEditsDictionary;
- (void)_initializeFromEditsDictionary;
- (_Bool)_isDraftFiltered;
- (_Bool)_hasUpdatedFilter;
- (_Bool)_isDraftWorthWarningBeforeDiscard;
- (void)_saveAsDraftAndDiscardWithLoggingExtraDict:(id)arg1 inView:(id)arg2;
- (void)_didTapCancel;
- (void)_togglePlayback;
- (void)_onVideoTapped;
- (void)_logBackwardsSessionEnd;
- (void)_logSessionEnd;
- (id)_chevronButtonWithIcon:(id)arg1;
- (void)_chevronTapped:(id)arg1;
@property(readonly, nonatomic) UIView<IGVideoPlaybackViewType> *playerView;
- (struct CGRect)_videoDisplayFrame;
- (id)_accessibilityHintForView:(id)arg1;
- (id)_accessibilityLabelForView:(id)arg1;
- (void)_configureAccessibilityForTabBar:(id)arg1;
- (void)_updateModeTabBarButtons;
- (void)_buildToggleToolsTabBar;
- (void)_buildToolsTabBar;
- (void)_buildToolContainerView;
- (void)_buildNavbar;
- (void)_updateCropRectFromCropView;
- (void)cropView:(id)arg1 didFinishAnimationWithCropRect:(struct CGRect)arg2;
- (void)cropViewVisibleContentFrameDidChange:(struct CGRect)arg1;
- (void)galleryAssetFetchRequest:(id)arg1 didFetchMediaMetadata:(id)arg2;
- (void)galleryAssetFetchRequest:(id)arg1 didUpdateProgress:(double)arg2;
- (void)galleryAssetFetchRequest:(id)arg1 didFailWithError:(id)arg2;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewWillDisappear:(_Bool)arg1;
- (void)_viewWillAppearInner:(_Bool)arg1;
- (void)viewWillAppear:(_Bool)arg1;
- (void)viewDidLayoutSubviews;
- (void)viewDidLoad;
- (void)_applicationWillResignActive:(id)arg1;
- (_Bool)allowsNavigationGestureForGestureOrigin:(struct CGPoint)arg1;
- (_Bool)prefersStatusBarHidden;
- (void)_trimClips;
- (id)initWithGalleryFetchRequest:(id)arg1 origin:(long long)arg2 previewImage:(id)arg3 useFbIgluFilters:(_Bool)arg4 userSession:(id)arg5 logger:(id)arg6;
- (id)initWithOrigin:(long long)arg1 videoInfo:(id)arg2 mediaMetadata:(id)arg3 isNewGalleryFlow:(_Bool)arg4 shareListManager:(id)arg5 useFbIgluFilters:(_Bool)arg6 userSession:(id)arg7 logger:(id)arg8;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

