//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import "IGStoryViewerHeaderAttributionType-Protocol.h"

@class IGStoryAttributionModel, IGUser, NSString;

@interface IGStoryAttributionFacebookUpsellAttributionController : NSObject <IGStoryViewerHeaderAttributionType>
{
    _Bool _isDisabledAttributionType;
    IGStoryAttributionModel *_attributionModel;
    IGUser *_sender;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) IGUser *sender; // @synthesize sender=_sender;
@property(readonly, nonatomic) IGStoryAttributionModel *attributionModel; // @synthesize attributionModel=_attributionModel;
- (_Bool)userInteractionEnabled;
- (id)newAttributionView;
- (void)didTakeActionWithTitle:(id)arg1 attributionDelegate:(id)arg2 navigationController:(id)arg3;
- (_Bool)shouldBeIncludedInMultiAttributionBottomSheet;
- (id)analyticsString;
- (_Bool)alwaysShowActionSheetOnTap;
- (id)tooltipProvider;
- (id)actionSheetButtonTitles;
- (id)initWithAttributionModel:(id)arg1 isDisabledAttributionType:(_Bool)arg2 sender:(id)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

