//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Mar 27 2019 15:10:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import "IGDirectMessageBlurable-Protocol.h"
#import "IGDirectMessageViewModelProtocol-Protocol.h"
#import "IGDirectMessageViewModelWithMediaProtocol-Protocol.h"
#import "IGDirectReshareMessageHeaderViewModel-Protocol.h"
#import "IGListDiffable-Protocol.h"

@class IGDirectMessageBlurredImageViewModel, IGDirectMessageCellViewModel, IGDirectPostMessageBubbleViewModel, IGDirectReshareDiffIdentifier, IGDirectTextMessageBubbleViewModel, IGDirectUIMessageMetadata, IGImageURL, IGMedia, NSString;
@protocol IGSponsoredInfoProviding, IGUserLauncherSet;

@interface IGDirectPostMessageViewModel : NSObject <IGDirectMessageBlurable, IGDirectMessageViewModelProtocol, IGDirectReshareMessageHeaderViewModel, IGListDiffable, IGDirectMessageViewModelWithMediaProtocol>
{
    struct CGSize _imageSize;
    struct CGSize _contentSize;
    double _cellHeight;
    NSString *_title;
    IGDirectReshareDiffIdentifier *_diffId;
    id <IGUserLauncherSet> _launcherSet;
    _Bool _shouldShowCallButtons;
    IGDirectUIMessageMetadata *_messageMetadata;
    IGDirectMessageCellViewModel *_messageCellViewModel;
    NSString *_contentType;
    NSString *_footerText;
    IGImageURL *_imageURL;
    IGDirectTextMessageBubbleViewModel *_captionTextModel;
    IGDirectTextMessageBubbleViewModel *_commentTextModel;
    IGMedia *_media;
    id <IGSponsoredInfoProviding> _sponsoredInfoProvider;
    NSString *_postItemPk;
    IGDirectMessageBlurredImageViewModel *_blurredImageViewModel;
    IGDirectPostMessageBubbleViewModel *_bubbleViewModel;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) _Bool shouldShowCallButtons; // @synthesize shouldShowCallButtons=_shouldShowCallButtons;
@property(readonly, nonatomic) IGDirectPostMessageBubbleViewModel *bubbleViewModel; // @synthesize bubbleViewModel=_bubbleViewModel;
@property(retain, nonatomic) IGDirectMessageBlurredImageViewModel *blurredImageViewModel; // @synthesize blurredImageViewModel=_blurredImageViewModel;
@property(readonly, nonatomic) NSString *postItemPk; // @synthesize postItemPk=_postItemPk;
@property(readonly, nonatomic) id <IGSponsoredInfoProviding> sponsoredInfoProvider; // @synthesize sponsoredInfoProvider=_sponsoredInfoProvider;
@property(readonly, nonatomic) IGMedia *media; // @synthesize media=_media;
@property(readonly, copy, nonatomic) IGDirectTextMessageBubbleViewModel *commentTextModel; // @synthesize commentTextModel=_commentTextModel;
@property(readonly, nonatomic) IGDirectTextMessageBubbleViewModel *captionTextModel; // @synthesize captionTextModel=_captionTextModel;
@property(readonly, copy, nonatomic) NSString *title; // @synthesize title=_title;
@property(readonly, nonatomic) IGImageURL *imageURL; // @synthesize imageURL=_imageURL;
@property(readonly, copy, nonatomic) NSString *footerText; // @synthesize footerText=_footerText;
@property(readonly, copy, nonatomic) NSString *contentType; // @synthesize contentType=_contentType;
@property(readonly, nonatomic) IGDirectMessageCellViewModel *messageCellViewModel; // @synthesize messageCellViewModel=_messageCellViewModel;
@property(readonly, nonatomic) IGDirectUIMessageMetadata *messageMetadata; // @synthesize messageMetadata=_messageMetadata;
- (_Bool)isEqualToDiffableObject:(id)arg1;
- (id)diffIdentifier;
@property(readonly) unsigned long long hash;
- (_Bool)isEqual:(id)arg1;
- (id)module;
- (unsigned long long)headerImageBadgeImageAsset;
- (id)usersInHeaderFacepile;
- (id)headerImageSpecifier;
- (id)reshareSubtitle;
- (id)reshareTitle;
- (long long)layoutType;
- (id)bubbleStyle;
- (_Bool)isChevronHidden;
- (_Bool)senderIsCurrentUser;
@property(readonly, nonatomic) long long tagIndicator;
@property(readonly, nonatomic) _Bool displaysSidecarIndicator;
@property(readonly, nonatomic) _Bool displaysVideoIndicator;
@property(readonly, nonatomic) struct CGSize imageSize;
@property(readonly, nonatomic) struct CGSize contentSize;
- (double)contentHeight;
- (id)initWithPostMessage:(id)arg1 messageCellViewModel:(id)arg2 shouldShowCallButtons:(_Bool)arg3 recipient:(id)arg4 launcherSet:(id)arg5;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) Class superclass;

@end

